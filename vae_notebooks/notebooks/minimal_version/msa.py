...
class MSA:
    ...
    def binary_to_numbers_coding(binary):
        ...
        Example: [[0,1,0],[0,0,1]] -> [1,2]
        """
        # Check if binary is a PyTorch tensor
        if isinstance(binary, torch.Tensor):
            binary = binary.cpu().numpy()  # Move to CPU and convert to NumPy array
    ...
    def binary_to_tensor(binaries) -> torch.Tensor:
        ...
        Get binary reshaped for vae
        """
        assert len(binaries.shape) == 2  # only for batches
        binaries = binaries.astype(np.float32)
        binaries = binaries.reshape((-1))
        return torch.from_numpy(binaries)
