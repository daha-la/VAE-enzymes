...
from notebooks.minimal_version.preprocess_msa import Preprocessor as Convertor
...
class MsaHandler(metaclass=Singleton):
    """
    ...
    def binaries_to_numbers_coding(binaries):
        ...
        [[0,1,0],[0,0,1],
         [0,1,0],[0,0,1]] -> [[1,2],[1,2]]
        """
        return np.array([MsaHandler.binary_to_numbers_coding(binary) for binary in binaries])
    ...
    def add_condition(x, c):
        ...
        if c is not None:
            c = MsaHandler.idx2onehot(c, n=SolubilitySetting.SOLUBILITY_BINS.value, x=x)
            if x.dim() == 1:
                x = x.unsqueeze(0)
